...
def test_data_matrix() -> None:
    ...
    fit_matrix = regression_noop.data_matrix(x, slice(nbases), covariates, nodes, hinges, where)
...
def extend_case(nbases: int,
    ...
    for c in [0.5, 0.4, 0.3]:
        ...
        nbases = regression_noop.add_bases(nbases, additional_covariates, additional_nodes, additional_hinges,
...
def shrink_case(nbases: int,
    ...
    for i in range(3):
        ...
        nbases = regression_noop.remove_bases(nbases, where, removal_slice)
...
def test_update_fit_matrix() -> None:
    ...
    def update_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, old_node, parent_idx):
        init_results = regression_noop.update_init(x, old_node, parent_idx, nbases, covariates, nodes, where,
    ...
    full_fit_matrix = regression_noop.calculate_fit_matrix(x, nbases, covariates, nodes, hinges, where)
...
def test_extend_fit_matrix() -> None:
    ...
    def extend_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, nadditions):
        ...
        fit_results[2] = regression_noop.extend_fit_matrix(x, nadditions, nbases, fit_results[2], covariates, nodes,
                                                           hinges, where)
        return fit_results
...
def test_update_covariance_matrix() -> None:
    ...
    def update_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, old_node, parent_idx):
        ...
        regression_noop.update_fit_matrix(fit_results[2], *init_results[:2])
        covariance_addition = regression_noop.update_covariance_matrix(fit_results[3], init_results[1], fit_results[2],
    ...
    full_fit_matrix = regression_noop.calculate_fit_matrix(x, nbases, covariates, nodes, hinges, where)
    full_covariance, _, _ = regression_noop.calculate_covariance_matrix(full_fit_matrix)
...
def test_extend_covariance_matrix() -> None:
    ...
    def extend_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, nadditions):
        ...
        fit_results[2] = regression_noop.extend_fit_matrix(x, nadditions, nbases, fit_results[2], covariates, nodes,
                                                           hinges, where)
        fit_results[3], fit_results[-3], fit_results[-2] = regression_noop.extend_covariance_matrix(fit_results[3],
                                                                                                    nadditions,
                                                                                                    fit_results[2],
                                                                                                    fit_results[-3],
                                                                                                    fit_results[-2])
    ...
    full_fit_matrix = regression_noop.calculate_fit_matrix(x, nbases, covariates, nodes, hinges, where)
    full_covariance, full_fixed_mean, full_candidate_mean = regression_noop.calculate_covariance_matrix(full_fit_matrix)
...
def test_update_right_hand_side() -> None:
    ...
    def update_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, old_node, parent_idx):
        init_results = regression_noop.update_init(x, old_node, parent_idx, nbases, covariates, nodes, where,
                                                   fit_results[2], fit_results[-2])
        regression_noop.update_right_hand_side(fit_results[5], y, fit_results[-1], *init_results[:2])
        return fit_results
    ...
    full_fit_matrix = regression_noop.calculate_fit_matrix(x, nbases, covariates, nodes, hinges, where)
    full_right_hand_side, _ = regression_noop.calculate_right_hand_side(y, full_fit_matrix)
...
def test_extend_right_hand_side() -> None:
    ...
    def extend_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, nadditions):
        ...
        fit_results[2] = regression_noop.extend_fit_matrix(x, nadditions, nbases, fit_results[2], covariates, nodes,
                                                           hinges, where)
        fit_results[5] = regression_noop.extend_right_hand_side(fit_results[5], y, fit_results[2], fit_results[-1],
                                                                nadditions)
        return fit_results
...
def test_decompose() -> None:
    ...
    nodes[2, 2] = new_node
    init_results = regression_noop.update_init(x, old_node, 1, nbases, covariates, nodes, where,
                                               fit_results[2], fit_results[-2])
...
def test_update_cholesky() -> None:
    ...
    def update_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, old_node, parent_idx):
        ...
        if covariance_addition.any():
            eigenvalues, eigenvectors = regression_noop.decompose_addition(covariance_addition)
            fit_results[4] = regression_noop.update_cholesky(fit_results[4], eigenvectors, eigenvalues)
        return fit_results
    ...
    full_fit_results = regression_noop.fit(x, y, nbases, covariates, nodes, hinges, where, 3)
...
def test_update_fit() -> None:
    ...
    def update_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, old_node, parent_idx):
        fit_results = list(fit_results)
        fit_results[:-1] = regression_noop.update_fit(x, y, nbases, covariates, nodes, where, 3, fit_results[2],
...
def test_extend_fit() -> None:
    ...
    def extend_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, nadditions):
        fit_results = list(fit_results)
        fit_results[:-1] = regression_noop.extend_fit(x, y, nbases, covariates, nodes, hinges, where, 3, nadditions,
                                                 fit_results[2],
                                                 fit_results[3], fit_results[5], fit_results[-1], fit_results[-3],
                                                 fit_results[-2])
        return fit_results
...
def test_shrink_fit() -> None:
    ...
    def shrink_func(x, y, nbases, covariates, nodes, hinges, where, fit_results, removal_slice):
        fit_results = list(fit_results)
        fit_results[:-1] = regression_noop.shrink_fit(y, nbases, 3, removal_slice, fit_results[2],
...
def test_expand_bases() -> None:
    ...
    model = regression_noop.OMARS(nbases, covariates, nodes, hinges, where, np.zeros(11))
    print(model)
    assert match1
    assert match2
...
def test_prune_bases() -> None:
    ...
    nbases, where, coefficients = regression_noop.prune_bases(x, y, nbases, covariates, nodes, hinges, where,
                                                              fit_results[0], fit_results[2], fit_results[3],
                                                              fit_results[5],
                                                              fit_results[-3], fit_results[-2], 3)

    ref_model = regression_noop.OMARS(ref_nbases, ref_covariates, ref_nodes, ref_hinges, ref_where, fit_results[1])
    test_model = regression_noop.OMARS(nbases, covariates, nodes, hinges, where, coefficients)
